...
void svgOpenTag(char path[]){
    ...
    fprintf(svg,"<svg viewBox=\"-200 -200 20000 20000\" style=\"width: 20000px; height: 20000px\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n");
    fprintf(svg,"<defs>\n");
    svgPrintFilter(svg);
    fprintf(svg,"</defs>\n");
    fclose(svg);
...
void _svgPrintCaminhoAnimado(FILE *svg, Poligono caminho){
    ...
    fprintf(svg,"<image x=\"\" y=\"\" width=\"45\" height=\"45\" href=\"%s\">\n", getVandaoSalvaVidas());
    fprintf(svg,"<animateMotion dur=\"%ds\" repeatCount=\"indefinite\">\n",len);
...
void svgSelectTag(Generic elemento, void *file){
    ...
    }else if(!strcmp(tipo, "caminho")){
        _svgPrintCaminhoAnimado(svg, genericGetValores(elemento));
    }
}
